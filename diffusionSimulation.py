...
dx = L / N  # increment between discrete points

x = np.linspace(-L/2, L/2, N, endpoint=False)  # x and y arrays defined based on N and L
y = np.linspace(-L/2, L/2, N, endpoint=False)
X, Y = np.meshgrid(x, y, indexing='ij')  # arrays explicitly turned into a meshgrid
...
sigma = 1
u0 = np.exp(-((X - 2)**2 + (Y - 2)**2) / (2 * sigma**2)) + np.exp(-((X + 2)**2 + (Y + 2)**2) / (2 * sigma**2)) + np.exp(-((X - 2)**2 + (Y + 2)**2) / (2 * sigma**2)) + np.exp(-((X + 2)**2 + (Y - 2)**2) / (2 * sigma**2))  # four gaussian hotspots with width sigma for initial conditions
u_hat = np.fft.fftn(u0)  # initial conditions transformed into fourier space
time_operator = np.exp(-alpha * k_squared * dt)  # Fourier transformed diffusion equation
