...
def simulate(
    ...
    D=0.1,  # 0.1
    dt=0.01,  # 0.01
    total_time=100000.0,  # 100000.0
    move_scale=0.0001,  # 0.0001 # step length per dt
    plot_interval=50,  # 50
    recovery_time=30.0,  # 30 # mean infection duration
    recovery_spread=0.2,  # 0.2 # ±20% variability
    immunity_multiplier=1.0,  # 1.0 # immune = multiplier × individual recovery_time
    station_rel=(0.9, 0.1),  # (0.9, 0.1)
    station_radius=0.1,  # 0.1
    vacc_prob=0.05,  # 0.05
    beta=0.005,  # 0.005 # mean transmissibility
    beta_spread=0.2,  # 0.2 # ±20% variability
    min_seg_steps=50,  # 50 # min length of straight segment
    max_seg_steps=600,  # 600 # max length of straight segment
    record_history=False,
    rng=None,  # optional deterministic seed
):
    ...
    for step in range(1, steps + 1):
        ...
        # 1) linear-segment motion + elastic walls
        ...
        # 2) deposit infection strength on grid (NO normalisation)
        ...
        t_inf[new_infected] = 0.0

        # 4) recovery → temporary immunity
        t_inf[infected] += dt
